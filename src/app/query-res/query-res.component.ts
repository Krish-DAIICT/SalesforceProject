...
export class QueryResComponent implements AfterViewInit{
  ...
  isRelationshipField(fieldName: string): any {
    const objectMetadata = this.salesforceService.getObjectMetadata(this.selectedObject); // Implement this method
  ...
  updateRecord(url: string): void{
    this.salesforceService.querySalesforce(this.selectedObject, this.selectedFieldsArray).subscribe({
      next: ((val) => {
